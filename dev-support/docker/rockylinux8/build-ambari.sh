...
build_command="mvn -B clean install rpm:rpm -DnewVersion=${new_version} -DbuildNumber=${build_number} -DskipTests -Dpython.ver='python >= 2.6'"

ambari_comile_docker_name='ambari-build-rockylinux8'
if [[ -z $(docker ps -a --format "table {{.Names}}" | grep ${ambari_comile_docker_name}) ]]; then
  docker run -itd --name ${ambari_comile_docker_name} \
    --network host \
    ambari/develop:trunk-rockylinux8
else
